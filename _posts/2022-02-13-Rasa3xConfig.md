...
Model Configuration
配置文件定义了模型根据用户输入进行预测的组件和策略。recipe 键允许不同类型的配置和模型架构。目前，只支持“default.v1”。语言键和管道键指定模型用于进行NLU预测的组件。Policys键定义了模型用于预测下一个操作的策略。如果您不知道要选择哪些组件或策略，可以使用建议的配置功能，这将推荐合理的默认设置。

Suggested Config
您可以将管道 或策略key保留在配置文件之外。运行rasa训练时，建议的配置功能将为缺失的key选择默认配置，以训练模型。确保在配置中指定语言键。带有两个字母ISO语言代码的config.yml文件。
...
如果愿意，可以取消对一个或两个键的建议配置的注释，并进行修改。请注意，这将在再次训练时禁用对此键的自动建议。只要您将配置注释掉，并且自己没有为key指定任何配置，无论何时训练新模型，都会建议使用默认配置。

NLU-或仅对话模式
如果运行rasa train nlu，则只会自动选择管道的默认配置；如果运行rasa train core，则只会选择策略的默认配置。
