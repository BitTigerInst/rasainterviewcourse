...
当你用这个版本调用rasa init时，你将能够开始一个快速的演示项目，你也可以立即训练。您将注意到.rasa文件夹包含一个名为cache.db的SQLite文件，以及在训练图中表示缓存结果的文件和文件夹。

您可以检查这些文件，以确认组件数据已被缓存。当您更改config.yml配置时，某些组件不会训练。 如果您在config.yml配置中添加RegexEntityExtractor，就可以从日志中确认DIETClassifier没有重新训练。在Rasa 2。x中最后训练的模型将是缓存的模型状态，而现在我们在所有组件上都有一个单独的全局缓存。  图架构一个主要的变化是，需要注册组件，以便图能够识别它们，从长远来看，这个新的图架构后台将带来很多好处。
