...
使用NLU管道，我们可以检测意图和实体亦，但是这个管道不能预测对话中的下一个动作。这就是策略管道的目的，策略利用NLU的预测以及到目前为止的对话状态来预测下一步要采取什么行动。

如果你对这些策略是如何产生行动感兴趣的，请欣赏Rasa的博客文章 https://rasa.com/blog/dialogue-policies-rasa-2/

我们回顾了Rasa NLU管道中的组件如何相互作用，了解组件在管道中的交互方式是很好的，因为这将帮助您确定哪些组件与您的助理相关。

同样重要的是要了解您可以完全定制管道。如果不需要组件，可以删除它们；如果想使用自己的工具，甚至可以编写自己的组件。如果您使用的是非英语语言，并且希望使用您熟悉的自定义语言工具，则这一点尤其重要。如果您有兴趣看到这样的例子，我们建议您查看rasa nlu示例存储库https://rasahq.github.io/rasa-nlu-examples/。这个存储库有许多标记器、特性化器和模型，您可以从中自由地为自己的项目获取灵感。
Happy Hacking!
