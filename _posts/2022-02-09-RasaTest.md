...
这将在您在带有test_前缀的文件中定义的任何端到端故事上测试您的最新训练模型。如果要使用不同的模型，可以使用–model标志指定它。
如果要分别评估对话和NLU模型，可以使用以下命令：


您可以在评估NLU模型和评估核心模型中找到更多详细信息。
以下参数可用于rasa测试：


rasa data split
要创建NLU训练数据的训练集、测试集 ，请运行：

默认情况下，这将按80/20的比例分割训练/测试数据。可以使用以下参数指定训练数据、比例和输出目录：


如果您有用于检索操作的NLG数据，则这些数据将保存到单独的文件中
...
YAML
JSON
您可以通过运行以下命令来启动转换器： 使用以下参数指定输入文件或目录、输出文件或目录以及输出格式：


运行一个示例
(installingrasa) E:\starspace\my_rasa>rasa data convert nlu --format json --data E:\starspace\my_rasa\data\nlu.yml
nlu.yml的格式
